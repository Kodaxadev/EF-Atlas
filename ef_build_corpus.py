#!/usr/bin/env python3
"""
EVE Frontier Builder Knowledge Corpus builder.

Layer 1:
  - Scrape official docs + whitepaper from their sitemap.md pages.

Layer 2:
  - Ingest selected repos into the same JSONL corpus.
...
from efk.layer2_repos import scrape_layer2_repos
from efk.records import count_by, utc_now_iso
from efk.urls import is_blocked_query_url
from efk.synthesis import write_synthesis
...
def main() -> int:
    ...
    # Network knobs (stdlib-only).
    timeout_s = 30.0
    polite_delay_s = 0.5
    max_retries = 4
    base_delay_s = 0.75
    user_agent = "EVEFrontierBuilderKnowledgeCorpus/1.0 (+https://docs.evefrontier.com/)"

    # Layer 1
    layer1_records, layer1_manifest = scrape_layer1(
        timeout_s=timeout_s,
    ...
    layers.append(layer1_manifest)

    # Layer 2
    layer2_records, layer2_manifest = scrape_layer2_repos(repos_dir=REPOS_DIR)
    layers.append({k: v for k, v in layer2_manifest.items() if k != "failures"})
    failures.extend(layer2_manifest.get("failures", []))

    records = [*layer1_records, *layer2_records]
    ...
    # Synthesis markdown (no opinions; indexed links only).
    write_synthesis(SYNTHESIS_DIR, CORPUS_PATH)
